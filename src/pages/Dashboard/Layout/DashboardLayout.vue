...
.wrapper(:class="[{ 'nav-open': $sidebar.showSidebar }]")
  side-bar(:active-color='sidebarBackground', :background-image='sidebarBackgroundImage', :data-background-color='sidebarBackgroundColor')
    ...
    template(slot='links')
      ...
      sidebar-item(v-if="$user.can('view-user')", :link="{ name: '客户管理', icon: 'people', path: '/user/customer' }")
      sidebar-item(v-if="$user.can('view-staff')", :link="{ name: '职员管理', icon: 'manage_accounts', path: '/user/staff' }")
