...
.wrapper(:class="[{ 'nav-open': $sidebar.showSidebar }]")
  ...
  side-bar(:active-color='sidebarBackground', :background-image='sidebarBackgroundImage', :data-background-color='sidebarBackgroundColor')
    ...
    template(slot='links')
      ...
      sidebar-item(:link="{ name: '活动预约', icon: 'event', path: '/booking?type=event' }")
      sidebar-item(:link="{ name: '礼品兑换', icon: 'card_giftcard', path: '/booking?type=gift' }")
      sidebar-item(:link="{ name: '餐饮消费', icon: 'fastfood', path: '/booking?type=food' }")
