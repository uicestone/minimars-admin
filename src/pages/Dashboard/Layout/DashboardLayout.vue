...
.wrapper(:class="[{ 'nav-open': $sidebar.showSidebar }]")
  ...
  side-bar(:active-color='sidebarBackground', :background-image='sidebarBackgroundImage', :data-background-color='sidebarBackgroundColor')
    ...
    template(slot='links')
      ...
      sidebar-item(v-if="$user.can('manage-card-types')", :link="{ name: '卡片种类', icon: 'card_membership', path: '/card-type' }")
      sidebar-item(v-if="$user.can('manage-coupon')", :link="{ name: '优惠管理', icon: 'confirmation_number', path: '/coupon' }")
