...
.wrapper(:class="[{ 'nav-open': $sidebar.showSidebar }]")
  side-bar(:active-color='sidebarBackground', :background-image='sidebarBackgroundImage', :data-background-color='sidebarBackgroundColor')
    ...
    template(slot='links')
      ...
      sidebar-item(v-if="$user.can('view-card-types')", :link="{ name: '卡片种类', icon: 'card_membership', path: '/card-type' }")
      sidebar-item(v-if="$user.can('view-coupon')", :link="{ name: '优惠管理', icon: 'confirmation_number', path: '/coupon' }")
      sidebar-item(v-if="$user.can('view-posts')", :link="{ name: '内容管理', icon: 'chrome_reader_mode', path: '/post' }")
      sidebar-item(v-if="$user.can('view-events')", :link="{ name: '活动管理', icon: 'event', path: '/event' }")
      sidebar-item(v-if="$user.can('view-gifts')", :link="{ name: '礼品管理', icon: 'card_giftcard', path: '/gift' }")
      sidebar-item(v-if="$user.can('view-payments')", :link="{ name: '流水明细', icon: 'payment', path: '/payment' }")
      sidebar-item(v-if="$user.can('view-stores')", :link="{ name: '门店管理', icon: 'store', path: '/store' }")
